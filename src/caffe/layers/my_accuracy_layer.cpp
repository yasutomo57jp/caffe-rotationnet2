...
#include "caffe/layers/my_accuracy_layer.hpp"
#include "caffe/util/math_functions.hpp"

namespace caffe {

template <typename Dtype>
void MyAccuracyLayer<Dtype>::LayerSetUp(
  const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  // has_ignore_label_ =
  //   this->layer_param_.my_accuracy_param().has_ignore_label();
  // if (has_ignore_label_) {
  //   ignore_label_ = this->layer_param_.my_accuracy_param().ignore_label();
  // }
...
void MyAccuracyLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  const int dim = nClassLabel * nRotation;
  ...
  for (int n = 0; n < nSample; ++n) {
	...
	for (int c = 0; c < nClassLabel - 1; ++c) {
	  ...
	  for (int j = 0; j < nRotation; ++j) {
	    ...
	    tmp_prob += log( bottom_data[ ( n * nRotation + idx ) * dim + j * nClassLabel + c ] ) - log( std::max(bottom_data[ ( n * nRotation + idx ) * dim + j * nClassLabel + nClassLabel - 1 ], Dtype(FLT_MIN)) );
	  }
	...
	for (int c = 0; c < nClassLabel - 1; ++c) {
	  Dtype tmp_prob = 0;
	  for (int j = 0; j < nRotation; ++j){
	    tmp_prob += log( bottom_data[ ( n * nRotation + ang[ i ][ j ] ) * dim + j * nClassLabel + c ] ) - log( std::max(bottom_data[ ( n * nRotation + ang[ i ][ j ] ) * dim + j * nClassLabel + nClassLabel - 1 ], Dtype(FLT_MIN)) );
	  }
	  if( tmp_prob > max_prob ){
	    max_ang = i;
	    max_prob = tmp_prob;
	...
	idx = ang[ max_ang ][ i ];
      ...
      for (int j = 0; j < nClassLabel - 1; ++j) 
	scores[ j ] += log( bottom_data[ ( n * nRotation + idx ) * dim + i * nClassLabel + j ] ) - log( std::max(bottom_data[ ( n * nRotation + idx ) * dim + i * nClassLabel + nClassLabel - 1 ], Dtype(FLT_MIN)) );
    }
...
INSTANTIATE_CLASS(MyAccuracyLayer);
REGISTER_LAYER_CLASS(MyAccuracy);
